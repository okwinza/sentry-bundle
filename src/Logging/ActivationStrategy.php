...
namespace CitePolitique\Sentry\Logging;
    ...
    private function isRecordActivated($record): bool
    {
        if (isset($record['context']['exception'])
            ...
            if ($exception instanceof HttpExceptionInterface
                && in_array($exception->getStatusCode(), $this->ignoredStatusCodes, true)) {
                return false;
            }

            if ($exception instanceof AccessDeniedException
                && in_array($exception->getCode(), $this->ignoredStatusCodes, true)) {
                return false;
            }
        }
